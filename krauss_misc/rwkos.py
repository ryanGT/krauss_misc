...
mac_roots = {'345':'/Users/kraussry/345_F24_local', \
             '445':'/Users/kraussry/445_545_SS24', \
...
default_figs_dir = os.path.expanduser("~/Work_vault_ios/345_prep/345_images")


def copy_figs_to_obsidian(pat="*", dst_dir=default_figs_dir, \
                           ext_list=['.pdf','.jpg','.png']):
    """Copy all figures in the current folder to dst_dir, which defaults to
...
def find_all_glob_matches(root, glob_list):
    if type(glob_list)  == str:
        glob_list = [glob_list]

    all_matches = []

    for glob_pat in glob_list:
        curmatches = Find_in_top_and_sub_dirs(root, \
                                        glob_pat)
...
def get_lecture_title(dir_in):
    ...
    if q106:
        ...
        q_num_only = re.match(r'\d\d\d/*', course_folder)
        if q_num_only:
...
def find_scanned_in_notes_pdf(dir_in):
    ...
    date_pat = re.compile(r'[IME]+_*\d\d\d_\d\d_\d\d_\d\d.pdf')
    matches = []
...
def DirsInThisLevel(pathin):
    ...
    dirs = [item for item in contents if \
            os.path.isdir(os.path.join(pathin,item))]
    return dirs
...
def FindInSubDirs(topdir, pat, skipdirs=[]):
    ...
    print("alldirs = %s" % alldirs)
    t2 = time.time()
    myfiles = []
    for curdir in alldirs:
        print("curdir = %s" % curdir)
        curroot = os.path.join(topdir, curdir)
        fullpat = os.path.join(curroot, pat)
...
def Find_in_top_and_sub_dirs(topdir, pat, skipdirs=[]):
    ...
    # note that FindInSubDirs only searches one level down
...
def find_in_list_of_paths(pathlist, pat):
    myfiles = []
    for curdir in pathlist:
...
def find_in_top_and_all_sub_dirs(topdir, pat, skipdirs=[]):
    # note that topdir should be included in alldirs
    # and that these should be abs paths
    all_dirs = FindAllSubFolders(topdir, skipdirs=skipdirs)
    all_files = find_in_list_of_paths(all_dirs, pat)
    return all_files
